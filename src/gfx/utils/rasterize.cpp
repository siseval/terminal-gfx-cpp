...
{
    Vec2d scaled_radius { Vec2d(radius) * scale };
...
void scan_line(std::shared_ptr<RenderSurface> surface, const int y, const Vec2i range, const std::vector<Box2d> edges, const Color4 color)
{
    ...
    {
        Vec2d pixel { static_cast<double>(x), static_cast<double>(y) };
        bool inside = false;
        for (auto edge : edges)
        {
...
// void barymetric_row(std::shared_ptr<RenderSurface> surface, const int y, const Vec2i range, const Triangle triangle, const Color4 color)
// {
//     std::vector<Vec2i> to_draw;
//     for (int x = range.x; x <= range.y; x++)
//     {
//         Vec2d pos { static_cast<double>(x), static_cast<double>(y) };
//
//         double area = 0.5 * (-triangle.v1.y * triangle.v2.x + triangle.v0.y * (-triangle.v1.x + triangle.v2.x) + triangle.v0.x * (triangle.v1.y - triangle.v2.y) + triangle.v1.x * triangle.v2.y);
//         if (area == 0)
//         {
//             continue;
//         }
//         double inv_area = 1 / (2 * area);
//         double s = inv_area * (triangle.v0.y * triangle.v2.x - triangle.v0.x * triangle.v2.y + (triangle.v2.y - triangle.v0.y) * pos.x + (triangle.v0.x - triangle.v2.x) * pos.y);
//         double t = inv_area * (triangle.v0.x * triangle.v1.y - triangle.v0.y * triangle.v1.x + (triangle.v0.y - triangle.v1.y) * pos.x + (triangle.v1.x - triangle.v0.x) * pos.y);
//
//         if (s >= 0 && t >= 0 && (s + t) <= 1)
//         {
//             to_draw.push_back(Vec2i { x, y });
...
//             for (int y = start_y; y <= end_y; y++)
//             {
//                 barymetric_row(surface, y, Vec2i { rounded_bounds.min.x, rounded_bounds.max.x }, { triangle.v0, triangle.v1, triangle.v2 }, color);
//             }
//         });
//     }
//     for (auto& thread : threads)
