...
public:
    ...
    Particle(std::shared_ptr<gfx::core::GfxRender2D> renderer, const gfx::math::Vec2d position, const gfx::math::Vec2d velocity, const gfx::math::Vec2d size, const gfx::core::types::Color4 color, const double lifespan)
    : renderer(renderer), position(position), velocity(velocity), size(size), color(color), lifespan(lifespan)
    {
        shape = renderer->create_ellipse(position, size, color);
        shape->set_fill(true);
